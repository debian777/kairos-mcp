...
The protocol has seven steps: (1) **Consult Claude Agent Skills specification via Context7** — fetch the official spec using library ID `/websites/platform_claude_en_agents-and-tools_agent-skills`; extract frontmatter rules, directory structure, progressive disclosure, and script best practices. (2) **Verify skill directory structure** — directory name matches `name`, SKILL.md present, optional reference/ scripts/ assets/; includes self-bootstrapping Python script pattern (venv auto-creation, `sys.prefix` detection, no external tools), convenience symlinks, and must-never rules for scripts. (3) **Gather requirements and draft SKILL.md** — purpose, triggers, location, and whether the skill has one protocol or multiple aliases; frontmatter and body; includes output directory guidance (cwd-anchored, visible vs dot-prefixed dirs), secrets handling pattern (`.secrets.{skill-name}` dotenv file, fallback env var names, auto-generated template, `test` subcommand with structured FAIL/ACTION output, AI-must-never-read rules), and gitignore guidance. (4) **Create protocol(s) via KAIROS** — for each alias (or single default), produce a protocol doc and save to `references/KAIROS.md` or `references/KAIROS-{alias}.md`. (5) **Mint protocol(s)** — for each existing `references/KAIROS.md` or `references/KAIROS-*.md`, strip frontmatter and call kairos_mint; report chain head URIs or skip. (6) **Verify by execution** — run the minted protocol(s) or skill in a subagent or new chat; report pass/fail. (7) **Lint skill** — run skills-ref validate. Use for personal (~/.cursor/skills/) or project (.cursor/skills/) skills.
