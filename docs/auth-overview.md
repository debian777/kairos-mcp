...
- **401 handling:** With **`--open`** (e.g. `kairos --open search "query"`), the CLI runs the same PKCE flow as `kairos login`, opens the browser, and retries after storing the token. Without `--open`, run `kairos login` then re-run the command.
