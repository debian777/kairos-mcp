...
export function buildChallengeShapeForDisplay(proof?: ProofOfWorkDefinition): Record<string, unknown> {
  const base: Record<string, unknown> = proof ? (() => {
    ...
    if (proofType === 'shell') {
      ...
      if (wd) {
        description += ` Use workdir "${wd}" as the process working directory after expanding env vars. This is real cwd behavior, not the shared local artifact directory. When a contract also refers to local handoff files, pick a hint from the latest activate/forward/next response field kairos_local_artifact_dir (an ordered list of project://<rel> and user://<rel> URIs), resolve it on your machine (project://<rel> -> <project root>/<rel>; user://<rel> -> <home or $XDG_CONFIG_HOME>/<rel>), and export KAIROS_LOCAL_ARTIFACT_DIR to the absolute path unless your shell already defines it. If the workdir path does not exist, is not a directory, or expansion leaves an empty path, fail with a clear local error.`;
      }
