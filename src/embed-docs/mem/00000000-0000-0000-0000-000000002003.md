...
Validate a drafted KAIROS protocol through format review, stranger review, user approval, and train. Invoked by Chain 1 (`create-new-protocol`) or directly to re-review an existing draft.

## Activation Patterns

**Typically invoked by:** Chain 1's chain link step via `activate(query: "Review and Publish New KAIROS Protocol")` or a stored adapter URI.
...
**If invoked directly** (no Chain 1), the engine creates `$KAIROS_WORK_DIR` when this adapter starts. Copy the draft file there before proceeding.
...
```json
{"contract":{"type":"user_input","user_input":{"prompt":"Review the protocol draft above. Type 'approved' to mint, describe adjustments, or 'cancel'."},"required":true}}
```
...
5. Successfully minted to KAIROS and discoverable via `activate`.
