...
- Run shell verification without a writable local artifact directory and
  `draft-protocol.md` under it. Use `$KAIROS_LOCAL_ARTIFACT_DIR` and the
  `kairos_local_artifact_dir` path from tool responses when exporting the shell.
...
`kairos_local_artifact_dir` with the canonical local handoff path for the run. Export
