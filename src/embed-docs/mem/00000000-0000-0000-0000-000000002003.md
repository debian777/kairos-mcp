...
- Run shell verification without a writable local artifact directory and
  `draft-protocol.md` under it. Prefer `$KAIROS_LOCAL_ARTIFACT_DIR`; the compat
  alias `$KAIROS_WORK_DIR` remains accepted only for compatibility.
...
run. The compat alias `KAIROS_WORK_DIR` / `kairos_work_dir` remains accepted,
but new adapters and docs must use the canonical name.
