...
- Run shell verification without a writable local artifact directory and
  `draft-protocol.md` under it. Use `$KAIROS_LOCAL_ARTIFACT_DIR`; if unset, pick
  a hint from the latest tool response field `kairos_local_artifact_dir`
  (ordered list of `project://<rel>` and `user://<rel>` URIs), resolve it on
  your machine, and export the absolute path before running shell challenges.
...
`kairos_local_artifact_dir` as an ordered array of URI hints (preferred first),
e.g. `["project://.local/kairos/work","user://.config/kairos/work"]`. Resolve
**on your machine**: `project://<rel>` → `<your project root>/<rel>`;
`user://<rel>` → `<your home or $XDG_CONFIG_HOME>/<rel>`. Use `project://` when
you have exactly one project context; fall through to `user://` when your
session spans multiple projects. Export `KAIROS_LOCAL_ARTIFACT_DIR="<absolute>"`
before shell challenges if the variable is not already set.
