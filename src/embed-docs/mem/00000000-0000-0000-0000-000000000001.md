...
```
# First, search for the minting guide to ensure proper structure
kairos_search(query: "minting kairos documents" or "mint protocol")

# Then mint using proper structure
kairos_mint(content: <document>)
```

**Note:** Always search for and follow the minting guide (`kairos_search("minting kairos documents")`) before creating new protocols to ensure proper challenge/solution structure.
