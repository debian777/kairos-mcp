...
def get_underlying_project_type(detection: ProjectDetection) -> str:
    """Get the underlying project type, ignoring Makefile (which is just a build wrapper).
    
    This is useful for validation purposes where we need to know the actual
    programming language/technology, not just the build tool.
    
    Returns: 'node', 'python', 'terraform', or 'other'
    """
    # Check actual project types, ignoring Makefile
    if detection.has_node:
        return "node"
    if detection.has_python:
        return "python"
...
def _get_project_type_keywords(project_type: str) -> List[str]:
    """Get expected keywords for a project type.
    
    Args:
        project_type: Project type ('node', 'python', 'terraform', 'other')
        Note: 'make' is not a valid project type here - it's just a build wrapper
        
    Returns:
        List of expected keywords (lowercase)
    """
    keywords_map = {
        "node": ["node", "nodejs", "typescript", "javascript", "npm", "ts", "js", "express", "next"],
        "python": ["python", "py", "django", "flask", "pytest", "pip"],
        "terraform": ["terraform", "tf", "infrastructure", "iac", "hcl"],
        "other": [],  # No specific keywords required
    }
    return keywords_map.get(project_type, [])


def _validate_library_name_for_project_type(library_name: str, project_type: str) -> Tuple[bool, Optional[str]]:
    """Validate that library_name contains project-type-related keywords.
    ...
    if project_type == "other":
        return True, None  # No validation for 'other' type
    
    expected_keywords = _get_project_type_keywords(project_type)
    if not expected_keywords:
        return True, None
    
    library_name_lower = library_name.lower()
    found_keywords = [kw for kw in expected_keywords if kw in library_name_lower]
    
    if not found_keywords:
        keywords_str = ", ".join(expected_keywords[:5])  # Show first 5
        return False, f"Library name '{library_name}' does not contain project-type-related keywords. Expected keywords for '{project_type}': {keywords_str}..."
...
def step_3_intelligence_augmentation(
    ...
    All verification data is stored in the step log file only.
    Validates that library_name contains project-type-related keywords.
    ...
    """
    step_number = 3
    step_title = "intelligence-augmentation"
    
    # Validate previous step log
    is_valid, error_msg = utils.validate_previous_step_log(root, step_number, step_title)
    if not is_valid:
        return False, error_msg
    ...
    # Validate library_name contains project-type-related keywords
    # Use underlying project type (ignoring Makefile, which is just a build wrapper)
    detection = project_detection.detect_project(root)
    project_type = project_detection.get_underlying_project_type(detection)
    is_valid_query, query_error = _validate_library_name_for_project_type(library_name, project_type)
    if not is_valid_query:
        return False, query_error

    # Make search API call with retries for 5xx errors
    ...
    # Retry configuration for 5xx errors: ~15 seconds total
    max_retries = 3
    retry_delays = [3, 5, 7]  # seconds between retries (total ~15 seconds)
    
    response = None
    
    for attempt in range(max_retries + 1):  # Initial attempt + 3 retries
        try:
            ...
            if 500 <= response.status_code < 600:
                if attempt < max_retries and attempt < len(retry_delays):
                    ...
                    errors.append(f"REST API call failed after {max_retries + 1} attempts")
                    output_lines.append(f"✗ REST API call failed after {max_retries + 1} attempts")
                    break
            
            # Other errors (4xx, etc.): don't retry
            errors.append("REST API call failed")
            output_lines.append("✗ REST API call failed")
            break
            ...
            if attempt < max_retries and attempt < len(retry_delays):
                ...
                errors.append("REST API call timed out")
                output_lines.append("✗ REST API call timed out")
                break
            ...
            errors.append("Unexpected error during REST API call")
            output_lines.append("✗ Unexpected error during REST API call")
            break
    
    # Check if we got a successful response
    if not response or response.status_code != 200:
        # Write log with error before returning
        log_content = f"""Step 3: Intelligence Augmentation (MCPs: Kairos & Context7)
Generated: {datetime.now(timezone.utc).isoformat()}
Library Name: {library_name}
Library IDs: {', '.join(library_ids) if library_ids else 'None'}
Search Results Count: 0
Found Library IDs: None
Missing Library IDs: {', '.join(library_ids) if library_ids else 'None'}
...
"""
    ...
    try:
        ...
        errors.append("Failed to parse API response")
        output_lines.append("✗ Failed to parse API response")
        log_content = f"""Step 3: Intelligence Augmentation (MCPs: Kairos & Context7)
Generated: {datetime.now(timezone.utc).isoformat()}
Library Name: {library_name}
Library IDs: {', '.join(library_ids) if library_ids else 'None'}
...
"""
    ...
    output_lines.append(f"✓ Search completed for '{library_name}': {search_results_count} results")
    ...
    if library_ids:
        ...
        for lib_id in library_ids:
            if lib_id in result_ids:
                found_library_ids.append(lib_id)
                output_lines.append(f"✓ Library ID found: {lib_id}")
            else:
                missing_library_ids.append(lib_id)
        ...
        success = True
        if all_results:
            output_lines.append(f"  Top result: {all_results[0].get('id', 'N/A')}")
    
    # Write step log (no JSON file - all data in log only)
    log_content = f"""Step 3: Intelligence Augmentation (MCPs: Kairos & Context7)
Generated: {datetime.now(timezone.utc).isoformat()}
Library Name: {library_name}
Library IDs: {', '.join(library_ids) if library_ids else 'None'}
Search Results Count: {search_results_count}
Found Library IDs: {', '.join(found_library_ids) if found_library_ids else 'None'}
Missing Library IDs: {', '.join(missing_library_ids) if missing_library_ids else 'None'}
Errors: {', '.join(errors) if errors else 'None'}
