...
**Must Never:**
- Modify any file, branch, or configuration outside
...
```text
{
  ...
  verdict_file: "$KAIROS_LOCAL_ARTIFACT_DIR/{verdict_file}"
}
```
...
2. Verify `$KAIROS_LOCAL_ARTIFACT_DIR` exists. If unset, export it from the
   latest `activate` / `forward` / `next` response field `local_artifact_dir`,
   or resolve locally to `$PROJECT_DIR/.local/kairos/work` if the returned path
   is not usable on your filesystem. `$KAIROS_WORK_DIR` is a deprecated compat
   alias and must not be used in new adapters/docs.
...
5. Write `$KAIROS_LOCAL_ARTIFACT_DIR/$CRITIC_VERDICT_FILE` so line 1 is `PASS`
   or `FAIL`,
...
```json
{"contract":{"type":"shell","shell":{"cmd":"test -n \"$CRITIC_VERDICT_FILE\" && test -f \"$KAIROS_LOCAL_ARTIFACT_DIR/$CRITIC_VERDICT_FILE\" && head -1 \"$KAIROS_LOCAL_ARTIFACT_DIR/$CRITIC_VERDICT_FILE\" | grep -qiE '^(PASS|FAIL)$' && sed -n '2p' \"$KAIROS_LOCAL_ARTIFACT_DIR/$CRITIC_VERDICT_FILE\" | grep -qiE '^(High|Medium|Low)$'","timeout_seconds":5},"required":true}}
```
