...
2. Verify `$KAIROS_LOCAL_ARTIFACT_DIR` exists. If unset, pick a hint from the
   latest `activate` / `forward` / `next` response field `kairos_local_artifact_dir`
   (an ordered array of `project://<rel>` and `user://<rel>` URIs), resolve on
   your machine (`project://<rel>` → `<your project root>/<rel>`;
   `user://<rel>` → `<your home or $XDG_CONFIG_HOME>/<rel>`), and export the
   absolute path.
