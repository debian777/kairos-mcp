...
2. Verify `$KAIROS_WORK_DIR` exists.
...
**Output:** structured audit report written to the verdict file
...
**Input:** audit findings and verdict file path
...
5. Write the verdict file so line 1 is `PASS` or `FAIL`, line 2 is confidence,
   and the remaining lines summarise key findings.
