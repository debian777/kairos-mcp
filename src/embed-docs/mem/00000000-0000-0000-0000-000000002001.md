...
This protocol is offered when activate finds no match.
...
**Must Never:**
- Run when the user only asked to run an existing protocol (use **activate** → **forward** (loop) → **reward**).
- Trigger when a strong match already exists from activate.
...
4. **General rules:**
  ...
  - The slug is a durable bookkeeping key. Natural Language Triggers and adapter URIs handle runtime discoverability and routing.
...
Call `train` with the approved markdown. Use `force_update: false`
...
- Is discoverable via `activate` (Natural Language Triggers)
- Uses appropriate challenge types per step
- If Router pattern: routes to extensions via `mcp` challenge with `activate` or a stored adapter URI
