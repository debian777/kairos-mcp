...
Ensure the local artifact directory is set before writing files. Use
`$KAIROS_LOCAL_ARTIFACT_DIR` if your shell already defines it; otherwise pick
a hint from the latest MCP response field `kairos_local_artifact_dir` (an
ordered list of `project://<rel>` and `user://<rel>` URIs), resolve it on
your machine (`project://<rel>` → `<your project root>/<rel>`;
`user://<rel>` → `<your home or $XDG_CONFIG_HOME>/<rel>`), then
`export KAIROS_LOCAL_ARTIFACT_DIR="<absolute>"`.

Produce a structured gap report at
`$KAIROS_LOCAL_ARTIFACT_DIR/protocol-review-findings.md`. For each finding
include:
