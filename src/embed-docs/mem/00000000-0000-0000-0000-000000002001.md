---
slug: create-new-kairos-protocol-chain
...
KAIROS protocol lifecycle — create, review, or fix adapter chains. Supports three operations:
...
**Trigger pattern:** **create** / **mint** / **review** / **audit** / **fix** / **update** / **patch** + (protocol / adapter / workflow / chain) [ + (new / existing / KAIROS) ].
...
- Enforce 350-line max per file. If content exceeds this, split into linked chains (via `mcp` challenge with `activate`) or extract a type extension / reference.
...
2. **Chaining gaps** — if the adapter performs an action covered by another adapter (branch creation, Jira lookup, commit formatting), does it chain to that adapter via `activate` or `forward`? Or does it silently skip the step, leaving the agent to improvise?
3. **DRY violations** — is policy content duplicated inline instead of chaining to the canonical source?
...
Produce a structured gap report. For each finding: adapter title, gap type (from the review checklist), severity (high/medium/low), recommended fix (specific: which layer to add, which `activate` call to insert, which reference to chain). Save to `$KAIROS_WORK_DIR/protocol-review-findings.md`.
...
Create protocols from generic to detailed. The generic chain comes first and must stand alone — it works even with zero extensions. Specializations (type checklists, domain extensions) are created second and link *back* to the generic via `activate(query: "...")` or a stored adapter URI.
...
**Challenge JSON format** (place at end of each step). Use a **single** concrete `type` per step (`comment`, `user_input`, `mcp`, or `shell`). Do not use pipe-separated placeholders inside `type` — each ` ```json ` block with `{"contract":...}` becomes its own chain step when minted.
...
Report the outcome: chain head URI(s) for create/fix, or findings summary for review.
