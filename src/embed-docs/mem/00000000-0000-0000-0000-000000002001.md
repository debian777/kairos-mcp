...
Ensure the local artifact directory is set before writing files. Prefer
`$KAIROS_LOCAL_ARTIFACT_DIR` from the latest MCP response field
`local_artifact_dir`. `$KAIROS_WORK_DIR` / `kairos_work_dir` are deprecated
compat aliases and are not acceptable in new protocol docs or adapters.
