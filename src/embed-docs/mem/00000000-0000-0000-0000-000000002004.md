...
- when using KAIROS MCP, read `local_artifact_dir` from the latest `activate` / `forward` / `next` response and `export KAIROS_LOCAL_ARTIFACT_DIR="<path>"` if your shell does not already define it
...
- `KAIROS_WORK_DIR` / `kairos_work_dir` are deprecated compat aliases and must
  not be used in new adapters/docs
