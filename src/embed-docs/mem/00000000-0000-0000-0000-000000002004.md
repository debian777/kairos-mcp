...
**Always use v2 format** when authoring new protocols or submitting solutions. The server accepts both formats:

- **v2 (preferred)**: `{type, outcome, evidence: {...}}` - unified envelope for all types
- **v1 (older)**: `{type, [type]: {...}}` - still accepted but never advertised

Key v2 improvements:
- Consistent structure across all challenge types
- `outcome` at top level instead of nested `success` boolean
- `evidence` envelope replaces type-specific objects
- MCP uses `response` instead of `result`

Existing protocols using v1 format continue to work unchanged. New protocols should follow v2 examples above.
