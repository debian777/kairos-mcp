...
- `kairos_local_artifact_dir` (optional on the start call) — canonical local artifact
  directory to keep stable for this run (same path as env `KAIROS_LOCAL_ARTIFACT_DIR`;
  the JSON key is lowercase snake of that env name). Use it when you want a project-local
...
optional `kairos_local_artifact_dir` (canonical local artifact directory for the run),
...
When shell contracts need local handoff files, use `kairos_local_artifact_dir` from the response and
