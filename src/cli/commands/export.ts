...
export function exportCommand(program: Command): void {
    program
        ...
        .option('--format <format>', 'markdown, trace_jsonl, sft_jsonl, or preference_jsonl', 'markdown')
        ...
        .action(async (uri: string, options: { format?: string; output?: string }) => {
            try {
                ...
                const response = await client.export(
                    uri,
                    (options.format as 'markdown' | 'trace_jsonl' | 'sft_jsonl' | 'preference_jsonl') ?? 'markdown'
                );
