#!/usr/bin/env node
/**
 * Run TypeScript check, Knip, and UI tests in parallel in CI; append job summary
 * for tsc and Knip after all finish (avoids concurrent writes to GITHUB_STEP_SUMMARY).
...
const [tsc, knip, ui] = await Promise.all([
...
const exitCode = tsc.code || knip.code || ui.code ? 1 : 0;
