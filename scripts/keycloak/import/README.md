...
5. **Test users** (dev only): `TEST_USERNAME` / `TEST_PASSWORD` (default `kairos-tester` / `kairos-tester-secret`), optional `KAIROS_CI_TEST_USERNAME` / `KAIROS_CI_TEST_PASSWORD` (default `kairos-ci-tester` / `kairos-ci-tester-secret`). The script ensures **`/shared/ci-test`** exists under the top-level **`shared`** group and assigns both users to **`ci-test`** (plus **`kairos-auditor`** for `kairos-tester`; **`kairos-operator`** only if the import lists top-level **`kairos-shares`**). Extra top-level groups not present in the import JSON are **deleted** from the realm so verification matches. After setting the password, each test user is **finalized** for **direct access grants** (non-temporary password, non-empty first/last name, verified email, empty **required actions**) so password grant does not return **Account is not fully set up** (Keycloak 24+).
