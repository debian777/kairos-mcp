...
## Introduction
This document explains the Export Tool implementation that creates and packages skill bundles into distributable protocol packages. It covers how export selections are interpreted, how artifacts and metadata are collected, how ZIP bundles are assembled, and how outputs are delivered via inline base64 or short-lived download references. It also documents input and output schemas, compression options, security controls, and best practices for sharing and distributing skills.
...
## Core Components
- Export orchestration: parses inputs, resolves selection, executes format-specific logic, and emits structured outputs.
- Input schema: validates mutually exclusive selection modes, format constraints, and delivery options.
- Output schema: defines fields for content, metadata, diagnostics, and download references.
...
- Skill assembly: collects SKILL.md and artifacts, derives metadata, appends checksums, and deduplicates slugs.
- Artifact loading: loads attached artifacts, normalizes relative paths, runs sanitization, and computes hashes.
- Download capability: mints short-lived, signed references for ZIP downloads with secure storage.
...
The Export Tool supports multiple output formats and delivery mechanisms. At runtime, the tool:
- Validates inputs and resolves adapter URIs
- Executes format-specific handlers (markdown, training JSONL, source, skill_tree, skill_zip)
- For skill_zip, assembles items, loads artifacts, computes checksums, and produces either inline base64 or a download reference
...
Zip-->>Tool : "Buffer(zip)"
Tool-->>CLI : "ExportOutput(content=base64)"
CLI-->>Client : "Write ZIP to file"
else "delivery=download_ref"
Tool->>Sec : "mintExportDownloadCapability(manifest)"
...
### Export Orchestration and Tool Registration
- Parses and validates inputs against the export schema.
- Resolves adapter URIs from single URI, adapter list, or space enumeration.
- Supports formats: markdown, training JSONL variants, source, skill_tree, skill_zip.
- Emits structured outputs with optional diagnostics, metadata, and download references.

```mermaid
flowchart TD
Start(["executeExport"]) --> Parse["Validate ExportInput"]
Parse --> Resolve["Resolve adapter URIs"]
...
Format --> |skill_tree| Tree["Assemble skill_tree JSON"]
Format --> |skill_zip| Bundle["Assemble items + artifacts"]
Bundle --> Delivery{"delivery"}
Delivery --> |inline_base64| Inline["zipSkillFiles -> base64"]
Delivery --> |download_ref| Ref["mintExportDownloadCapability"]
...
CLI->>Tool : "export(input)"
Tool-->>CLI : "ExportOutput(download_ref or inline_base64)"
alt "download_ref"
...
- Assembles each skill item with derived metadata and protocol content.
...
- Mints a short-lived, signed opaque token containing an expiration and payload.
...
participant Client as "Client"
Tool->>Cap : "mintExportDownloadCapability(input)"
Cap->>Store : "put(record, ttl)"
Cap-->>Tool : "{ url, expires_at, filename }"
Tool-->>Client : "download_ref.url"
Client->>Cap : "GET /export/skill-zip/{opaque}"
Cap->>Store : "get(id)"
Store-->>Cap : "record"
Cap-->>Client : "Stream ZIP bytes"
```
...
- Single adapter to flat markdown:
  ...
  - Output is a single-file markdown document.
- Multi-adapter bundle as skill_tree:
  - Provide an adapter list or export all adapters in a space.
  - Output is a JSON skill_tree with per-skill file lists and diagnostics.
- Multi-adapter bundle as skill_zip:
  - Choose skill_zip with delivery=download_ref to receive a short-lived URL.
  - Alternatively, choose delivery=inline_base64 to receive base64-encoded ZIP bytes.
- Training data export:
  - Select a single adapter URI and choose one of the training JSONL formats.
  - Output is NDJSON training pairs suitable for downstream training.

Package customization tips:
- Adjust KAIROS_EXPORT_ZIP_COMPRESSION_LEVEL to balance speed and size.
- Use space_name with all_adapters to export curated sets.
- Prefer download_ref for large bundles to avoid inline base64 overhead.

Distribution strategies:
- Share download_ref URLs with recipients who can access the platform.
- Distribute inline_base64 ZIPs for offline scenarios, ensuring recipient systems can decode safely.
- Include skill_bundle_manifest for automation and verification.
...
## Dependency Analysis
The Export Tool composes multiple subsystems with clear boundaries:
- Input validation depends on URI regexes and format enums.
- Selection resolution depends on space context and adapter enumeration.
- Skill assembly depends on metadata derivation, artifact loading, and checksum computation.
- Delivery depends on download capability signing and storage.
- ZIP streaming depends on compression settings and archiver library.
...
## Conclusion
The Export Tool provides a robust, validated pipeline to produce skill bundles in multiple formats. It supports flexible selection modes, secure delivery via short-lived download references, and efficient streaming ZIP packaging. By tuning compression, leveraging download_ref for large bundles, and following the input/output schemas, teams can reliably share and distribute skills.
...
- Manifest: skill_bundle_manifest (JSON string)
