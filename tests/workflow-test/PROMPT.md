...
2. **Search + workflows** — Start from natural language: call `kairos_run(message)` with a query that should match the minted chains (or use `kairos_search` then `kairos_begin` with the chosen URI). Then call `kairos_next` in a loop until `next_action` indicates run complete. Use protocols that do not require shell (MCP-only) if constraints forbid shell.
