/**
 * Use existing Keycloak from .env (KEYCLOAK_DEV_URL, etc.) for auth tests.
...
async function provisionTestUserOnExistingKeycloak(
  baseUrl: string,
  realm: string = REALM,
  adminUsername: string = process.env.KEYCLOAK_DEV_ADMIN_USERNAME ?? 'admin',
  adminPassword: string =
    process.env.KEYCLOAK_ADMIN_PASSWORD ??
    process.env.KEYCLOAK_DEV_ADMIN_PASSWORD ??
    'admin'
): Promise<void> {
  let token: string;
  try {
...
export async function useExistingKeycloakFromEnv(): Promise<KeycloakTestEnv> {
  const keycloakUrl = process.env.KEYCLOAK_DEV_URL?.replace(/\/$/, '') ?? '';
  const realm = process.env.KEYCLOAK_DEV_REALM ?? REALM;
  const clientId = process.env.KEYCLOAK_DEV_CLIENT_ID ?? CLIENT_ID;
  if (!keycloakUrl) {
    throw new Error(
      'useExistingKeycloakFromEnv requires KEYCLOAK_DEV_URL in .env.dev (e.g. http://localhost:8080)'
    );
  }
...
/**
 * Use existing QA Keycloak from .env.qa (KEYCLOAK_QA_URL). Provisions test user on QA realm.
...
export async function useExistingKeycloakForQa(): Promise<KeycloakTestEnv> {
  const keycloakUrl = process.env.KEYCLOAK_QA_URL?.replace(/\/$/, '') ?? '';
  const realm = process.env.KEYCLOAK_QA_REALM ?? QA_REALM;
  const clientId = process.env.KEYCLOAK_QA_CLIENT_ID ?? CLIENT_ID;
  if (!keycloakUrl) {
    throw new Error(
      'useExistingKeycloakForQa requires KEYCLOAK_QA_URL in .env.qa (e.g. http://localhost:8080)'
    );
  }
  const adminUsername = process.env.KEYCLOAK_QA_ADMIN_USERNAME ?? process.env.KEYCLOAK_DEV_ADMIN_USERNAME ?? 'admin';
  const adminPassword =
    process.env.KEYCLOAK_ADMIN_PASSWORD ??
    process.env.KEYCLOAK_QA_ADMIN_PASSWORD ??
    process.env.KEYCLOAK_DEV_ADMIN_PASSWORD ??
    'admin';
  await provisionTestUserOnExistingKeycloak(keycloakUrl, realm, adminUsername, adminPassword);
  const getTestUserToken = () =>
    getTestUserTokenFromKeycloak(keycloakUrl, realm, clientId, TEST_USERNAME, TEST_PASSWORD);
  return {
    keycloakUrl,
