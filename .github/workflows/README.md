...
2. **Publish image** — runs after npm succeeds; builds and pushes `jakub-plichcinski/kairos-mcp:<version>` and `latest` to Docker Hub, and `quay.io/<QUAY_NAMESPACE>/kairos-mcp:<version>` and `latest` to Quay.
